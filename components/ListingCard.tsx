...
const ListingCard: React.FC<ListingCardProps> = ({ listing, expanded = false, aiResponse, dragOffset = 0, isDragging = false, onImageClick }) => {
  ...
  return (
    <div className="relative bg-white rounded-lg shadow-lg overflow-hidden md:max-w-xl mx-auto cursor-grab active:cursor-grabbing" style={cardStyle}>
      ...
      />
      <div className="p-4">
        <h3 className="text-xl font-semibold text-gray-800 mb-1">{listing.title}</h3>
        ...
        {expanded && aiResponse && (
          <div className="mt-4 border-t pt-4">
            ...
            {aiResponse.after_right_swipe?.extra_fields.includes('description') && listing.description && (
              <p className="text-sm text-gray-700 mb-3">{listing.description}</p>
            )}
            {aiResponse.after_right_swipe?.extra_fields.includes('seller_name') && (
              <p className="text-sm text-gray-700 mb-1">Seller: {listing.seller_name}</p>
            )}
            {aiResponse.after_right_swipe?.extra_fields.includes('posted_at') && (
              <p className="text-sm text-gray-700 mb-3">Original Post Date: {postedDate}</p>
            )}

            {/* Always display follow-up questions if available */}
            {aiResponse.after_right_swipe?.follow_up_questions.length > 0 && (
              <div className="mt-4">
                <h5 className="text-md font-semibold text-gray-800 mb-2">Suggested Follow-up Questions:</h5>
                <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                  {aiResponse.after_right_swipe.follow_up_questions.map((q, index) => (
                    <li key={index}>{q}</li>
                  ))}
                </ul>
              </div>
            ...
            {aiResponse.risks && aiResponse.risks.length > 0 && (
              <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                <h5 className="text-md font-semibold text-yellow-800 mb-1">Potential Risks:</h5>
            ...
            {aiResponse.notes && aiResponse.notes.length > 0 && (
              <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
                <h5 className="text-md font-semibold text-blue-800 mb-1">Notes:</h5>
